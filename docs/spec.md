...
- 核心功能:
  ...
  - Prompt 支持字符数、复制、清空、快捷键生成与自适应高度；灵感模板保留内置模板，并支持在当前浏览器新增、编辑和删除自定义模板；结果按轮次分组并支持选择、对比、收藏、批量保留和批量下载；引用轮次使用“引用来源 → Prompt/参数 → 生成结果”链路卡片，可再次引用来源或任意结果
  ...
  - 每张生成结果和引用来源图支持 AI 分析气泡：调用 GPT-5.6 Terra/Sol/Luna 多模态流式对话，首轮发送 Base64 图片和原始生成 Prompt，支持快捷问题、自由追问、最多5轮、停止/重试和本地保存
  - 快捷分析使用 `<analysis>`/`<edit_prompt>` 结构化输出，分析正文与可执行修改 Prompt 分离；修改 Prompt 可独立编辑、复制、替换顶部 Prompt、引用当前图片继续或重新提炼；自由回答只能经“提炼为修改 Prompt”后用于生图
