        ...
        <section v-if="analysisOpen" ref="analysisBubble" class="analysis-bubble" :class="{ 'analysis-bubble--minimized': analysisMinimized }" :style="analysisBubbleStyle" aria-label="AI 图像分析">
            ...
            <template v-if="!analysisMinimized">
                ...
                <div ref="analysisConversation" class="analysis-conversation">
                    ...
                    <article v-for="(message, index) in analysisMessages" :key="index" :class="`analysis-message analysis-message--${message.role}`"><strong>{{ message.role === 'user' ? '你' : `AI · ${analysisModelName}` }}</strong><p>{{ message.content }}<span v-if="analysisStreaming && index === analysisMessages.length - 1 && message.role === 'assistant'" class="stream-caret"></span></p><div v-if="message.role === 'assistant' && message.content" class="analysis-answer-actions"><button type="button" @click="copyText(message.content)">复制</button><button type="button" @click="appendAnalysisToPrompt(message.content)">追加到 Prompt</button><button type="button" @click="replacePromptWithAnalysis(message.content)">替换 Prompt</button><button type="button" @click="adoptAnalysisAndContinue(message.content)">采用并引用继续</button></div></article>
                </div>
...
const savedAnalysisSessions = ref<Record<string, { model: VisionModelId; messages: VisionMessage[]; label: string }>>({});
const analysisPresets = [
    { label: '识别画面内容', prompt: '请描述图片中的主体、环境、动作、风格、构图、光线和主要细节。' },
    { label: '评价生成效果', prompt: '请从构图、色彩、光影、主体表现、细节完整度和视觉层级评价这张图片。' },
    { label: '给出修改建议', prompt: '请给出具体、可执行的图片修改建议，并按优先级排序。' },
    { label: '检查画面瑕疵', prompt: '请检查肢体、五官、文字、透视、边缘、重复元素、材质和光影是否存在异常。' },
    { label: '反推生成 Prompt', prompt: '请根据图片整理一份可直接用于图片生成模型的详细 Prompt，只输出 Prompt。' },
    { label: '生成精修 Prompt', prompt: '请在保持主体和构图的前提下，生成一份可直接用于继续精修当前图片的 Prompt，只输出 Prompt。' },
];
...
function persistAnalysisSession() {
    ...
    savedAnalysisSessions.value[analysisImage.value.id] = { model: analysisModel.value, messages: analysisMessages.value, label: analysisImageLabel.value };
...
async function sendAnalysisMessage() {
    ...
    try {
        await analyzeImageStream({ apiKey: apiKey.value, baseUrl: 'https://tokenverse.corp.kuaishou.com/v1', model: analysisModel.value, imageDataUrl: analysisImage.value.dataUrl, messages: analysisMessages.value.slice(0, -1), signal: analysisController.signal, onDelta: (delta) => {
...
function resetAnalysis() { stopAnalysis(); analysisMessages.value = []; analysisError.value = ''; analysisLastQuestion.value = ''; persistAnalysisSession(); }
function appendAnalysisToPrompt(content: string) { prompt.value = prompt.value ? `${prompt.value.trimEnd()}\n${content}` : content; }
function replacePromptWithAnalysis(content: string) { prompt.value = content; }
function adoptAnalysisAndContinue(content: string) {
    if (!analysisImage.value) return;
