                ...
                <div v-else class="image-grid" :class="`image-grid--${Math.min(generatedImages.length, 4)}`">
                    <article v-for="(image, index) in generatedImages" :key="image.id" class="image-card" :class="{ 'image-card--selected': selectedSourceImage?.id === image.id }">
                        <div class="image-thumb" @click="previewImage = image.dataUrl">
                            <img :src="image.dataUrl" :alt="`生成结果 ${index + 1}`" />
                            <span class="zoom-hint">点击放大</span>
                            <div class="prompt-tooltip">{{ getRoundPrompt(image.id) }}</div>
                        </div>
                        <footer>
...
function getRoundPrompt(imageId: string) {
    const round = sessionRounds.value.find((r) => r.images.some((img) => img.id === imageId));
    return round?.prompt || '';
}
...
<style scoped>
.studio-shell { display: flex; box-sizing: border-box; min-height: 100dvh; flex-direction: column; overflow: hidden; background: var(--bg_contain); color: var(--text_primary); padding: 16px 20px; }
.topbar { display: flex; flex: 0 0 auto; align-items: flex-start; justify-content: space-between; width: 100%; margin: 0 0 14px; }
...
.workspace { display: grid; min-height: 0; flex: 1; grid-template-columns: clamp(210px, 17vw, 280px) clamp(260px, 20vw, 330px) minmax(420px, 1fr); gap: 14px; width: 100%; }
...
.history-panel { display: flex; min-height: 0; flex-direction: column; padding: 16px 14px; overflow: hidden; }.history-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; padding: 4px 4px 14px; border-bottom: 1px solid var(--border_divider); }.history-header h2 { margin: 0 0 4px; }.history-header p { margin: 0; color: var(--text_tertiary); font-size: 12px; }.new-session-button { border: 1px solid var(--border_brand); border-radius: 5px; padding: 5px 8px; background: var(--bg_brand_contain); color: var(--text_brand); cursor: pointer; font-size: 12px; font-weight: 700; }.new-session-button:hover { background: var(--text_brand); color: #fff; }.history-empty { padding: 34px 10px; color: var(--text_tertiary); font-size: 13px; line-height: 1.7; text-align: center; }.history-empty small { font-size: 12px; }.session-list { display: flex; flex: 1; flex-direction: column; gap: 6px; overflow-y: auto; padding: 12px 0; }.session-item { width: 100%; border: 1px solid transparent; border-radius: 5px; padding: 10px; background: transparent; color: var(--text_primary); cursor: pointer; text-align: left; }.session-item:hover, .session-item--active { border-color: var(--border_divider); background: var(--bg_brand_contain); }.session-title { display: block; overflow: hidden; font-size: 14px; font-weight: 700; line-height: 1.35; text-overflow: ellipsis; white-space: nowrap; }.session-item small { display: block; margin-top: 4px; color: var(--text_tertiary); font-size: 12px; }.history-footer { display: flex; justify-content: space-between; gap: 8px; padding: 12px 4px 2px; border-top: 1px solid var(--border_divider); }.text-button--danger { color: var(--text_negative); }
.config-panel { padding: 18px 20px; overflow-y: auto; scrollbar-gutter: stable; } .section-heading { margin-bottom: 12px; }.section-heading p { margin-bottom: 0; }
...
.field-input:focus { border-color: var(--border_brand); box-shadow: 0 0 0 2px var(--bg_brand_tag); }.key-storage-actions { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-top: 6px; color: var(--text_tertiary); font-size: 11px; }.field-input:disabled { cursor: not-allowed; opacity: .55; }.field-hint { margin: 6px 0 0; color: var(--text_tertiary); font-size: 12px; }.url-presets { display: flex; gap: 10px; justify-content: flex-end; margin-bottom: -22px; }.text-button { border: 0; padding: 0; background: none; color: var(--text_brand); font: inherit; font-size: 12px; cursor: pointer; }.text-button:hover { text-decoration: underline; }.text-button:focus-visible, .generate-button:focus-visible, .close-button:focus-visible, .reference-item button:focus-visible { outline: 2px solid var(--border_brand); outline-offset: 2px; }
...
.result-panel { display: flex; min-height: 0; flex-direction: column; padding: 18px; overflow: hidden; }.prompt-bar { display: flex; flex: 0 0 auto; gap: 10px; align-items: flex-end; padding-bottom: 12px; border-bottom: 1px solid var(--border_divider); }.prompt-bar-input { flex: 1; min-height: 72px; max-height: 200px; resize: vertical; border: 1px solid var(--border_form); border-radius: 6px; padding: 10px 12px; background: var(--bg_component); color: var(--text_primary); font: inherit; font-size: 14px; line-height: 1.6; outline: none; transition: border-color .15s, box-shadow .15s; }.prompt-bar-input:focus { border-color: var(--border_brand); box-shadow: 0 0 0 2px var(--bg_brand_tag); }.prompt-bar .generate-button { flex: 0 0 auto; min-width: 72px; min-height: 72px; margin: 0; border: 0; border-radius: 6px; background: var(--text_brand); color: #fff; cursor: pointer; font-size: 14px; font-weight: 700; transition: opacity .15s, transform .15s; }.prompt-bar .generate-button:hover { opacity: .9; }.prompt-bar .generate-button:active { transform: scale(.97); }.prompt-bar .generate-button:disabled { cursor: not-allowed; opacity: .45; }.prompt-bar-meta { flex: 0 0 auto; padding-top: 8px; }.prompt-bar-meta .progress-box { margin-top: 0; }.prompt-bar-meta .progress-box p { display: none; }.result-header { display: flex; flex: 0 0 auto; align-items: flex-start; justify-content: space-between; padding: 12px 0 12px; border-bottom: 1px solid var(--border_divider); }.result-header p { margin-bottom: 0; }.result-state { display: flex; min-height: 0; flex: 1; flex-direction: column; align-items: center; justify-content: center; text-align: center; }.result-state strong { font-size: 15px; }.result-state p { max-width: 360px; margin: 8px 0 0; }.result-spinner { width: 22px; height: 22px; margin-bottom: 12px; color: var(--text_brand); }.image-grid { display: grid; min-height: 0; flex: 1; gap: 12px; padding-top: 14px; overflow: auto; align-content: start; }.image-grid--1 { grid-template-columns: minmax(0, 1fr); justify-items: center; }.image-grid--1 .image-card { width: min(100%, 520px); }.image-grid--2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }.image-grid--3, .image-grid--4 { grid-template-columns: repeat(2, minmax(0, 1fr)); }.image-card { overflow: hidden; border: 1px solid var(--border_divider); border-radius: 6px; background: var(--bg_layout); }.image-card--selected { border-color: var(--border_brand); box-shadow: 0 0 0 2px var(--bg_brand_tag); }.image-thumb { position: relative; cursor: zoom-in; }.image-thumb img { display: block; width: 100%; max-height: 240px; aspect-ratio: auto; object-fit: contain; background: var(--bg_contain); }.image-thumb:hover .zoom-hint { opacity: 1; }.zoom-hint { position: absolute; bottom: 6px; right: 6px; padding: 2px 7px; border-radius: 4px; background: rgba(0,0,0,.55); color: #fff; font-size: 10px; opacity: 0; transition: opacity .2s; pointer-events: none; }.prompt-tooltip { position: absolute; left: 50%; bottom: 100%; transform: translateX(-50%) translateY(-6px); max-width: 280px; max-height: 140px; overflow-y: auto; padding: 8px 11px; border-radius: 6px; background: rgba(0,0,0,.82); color: #fff; font-size: 12px; line-height: 1.5; white-space: pre-wrap; word-break: break-word; opacity: 0; visibility: hidden; transition: opacity .2s, visibility .2s; pointer-events: none; z-index: 10; }.image-thumb:hover .prompt-tooltip { opacity: 1; visibility: visible; }.image-card footer { display: flex; justify-content: space-between; align-items: center; padding: 7px 10px; color: var(--text_secondary); font-size: 12px; }.image-actions { display: flex; gap: 10px; align-items: center; }.image-actions button { border: 0; padding: 0; background: none; color: var(--text_brand); cursor: pointer; font: inherit; font-size: 12px; }.image-actions button:hover { text-decoration: underline; }.image-actions--active { color: var(--text_tertiary) !important; cursor: default !important; text-decoration: none !important; }.image-card a { color: var(--text_brand); text-decoration: none; }.image-card a:hover { text-decoration: underline; }.preview-overlay { position: fixed; z-index: 400; inset: 0; display: flex; align-items: center; justify-content: center; padding: 36px; background: rgba(0, 0, 0, .75); }.preview-overlay img { max-width: 100%; max-height: 100%; object-fit: contain; }.close-button { position: fixed; top: 16px; right: 18px; width: 38px; height: 38px; border: 0; border-radius: 6px; background: var(--bg_component); color: var(--text_primary); cursor: pointer; font-size: 24px; }
