        ...
        <section class="workspace" :class="{ 'workspace--history-collapsed': historyCollapsed }" data-module-id="image-generator-workbench">
            <aside v-if="!historyCollapsed" class="history-panel">
                ...
                <div v-if="sessions.length" class="history-footer">
                    <button type="button" class="text-button" @click="renameCurrentSession">重命名</button>
                    <button type="button" class="text-button" @click="exportCurrentSession">导出</button>
                    <label class="text-button import-button">导入<input type="file" accept="application/json" @change="importSession" /></label>
                    <button type="button" class="text-button text-button--danger" @click="deleteCurrentSession">删除</button>
                    <button type="button" class="text-button text-button--danger" @click="clearAllSessions">清空</button>
                </div>
            </aside>
            <aside class="config-panel">
                ...
                <p v-if="!isGemini && (selectedSourceImage || referencePreviews.length)" class="capability-notice">GPT-Image-2 当前使用 generations 接口，引用图片暂不会发送；待确认 edits 接口后启用。</p>
...
async function handleGenerate() {
    ...
    try {
        const result = await generateImage({
            ...
            count: count.value,
            referenceImages: [
                ...(selectedSourceImage.value && isGemini.value ? [sourceImageToReference(selectedSourceImage.value)] : []),
                ...(isGemini.value ? referencePreviews.value.map(({ mimeType, base64 }) => ({ mimeType, base64 })) : []),
            ],
