        ...
        <section class="workspace" :class="{ 'workspace--history-collapsed': historyCollapsed }" data-module-id="image-generator-workbench">
            ...
            <section class="result-panel" data-module-id="image-generation-results">
                ...
                <div class="template-launcher">
                    ...
                    <div v-if="templatesOpen" class="template-panel">
                        <button v-for="item in promptTemplates" :key="item.name" type="button" @click="applyPromptTemplate(item.prompt)">
                    ...
                    <section v-for="(round, roundIndex) in sessionRounds" :key="round.id" class="round-group">
                        ...
                        <div v-if="roundSourceUrl(round)" class="creation-chain">
                            <div class="chain-source">
                                ...
                                <button type="button" class="chain-action" @click="reuseRoundSource(round)">再次引用</button>
                            </div>
...
const promptTemplates = [
    { name: '摄影写实', description: '自然光、真实质感', prompt: '写实摄影风格，自然光线，真实材质与细节，主体清晰，构图专业，高动态范围。' },
    { name: '产品海报', description: '棚拍布光、商业构图', prompt: '商业产品海报，主体居中，干净背景，棚拍级布光，明确视觉层级，保留文案留白区域。' },
    { name: '人物肖像', description: '柔和轮廓、电影质感', prompt: '高质量人物肖像，神态自然，柔和轮廓光，细腻肤质，浅景深，电影感构图。' },
    { name: 'UI 概念图', description: '清晰层级、精致组件', prompt: '现代数字产品 UI 概念图，信息层级清晰，留白克制，组件精致，专业产品设计展示。' },
    { name: '编辑插画', description: '统一色彩、叙事画面', prompt: '精致编辑插画，统一色彩体系，轮廓清晰，画面富有叙事性，细节丰富且构图平衡。' },
];
