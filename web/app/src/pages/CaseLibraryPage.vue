      ...
      <div class="filter-groups">
        <div class="filter-row">
          ...
          <div class="filter-chips">
            <button type="button" :class="['filter-chip', { 'filter-chip--active': category === 'all' }]" @click="category = 'all'">全部</button>
            <button v-for="item in visibleCategories" :key="item" type="button" :class="['filter-chip', { 'filter-chip--active': category === item }]" @click="category = item">{{ categoryLabel(item) }}</button>
            <button v-if="categories.length > categoryLimit" type="button" class="filter-chip filter-chip--more" @click="showAllCategories = !showAllCategories">{{ showAllCategories ? '收起' : `更多 ${categories.length - categoryLimit}` }}</button>
          </div>
        </div>
        <div class="filter-row">
          <span class="filter-label">风格</span>
          <div class="filter-chips">
            <button type="button" :class="['filter-chip', { 'filter-chip--active': style === 'all' }]" @click="style = 'all'">全部</button>
            <button v-for="item in visibleStyles" :key="item" type="button" :class="['filter-chip', { 'filter-chip--active': style === item }]" @click="style = item">{{ item }}</button>
            <button v-if="styles.length > styleLimit" type="button" class="filter-chip filter-chip--more" @click="showAllStyles = !showAllStyles">{{ showAllStyles ? '收起' : `更多 ${styles.length - styleLimit}` }}</button>
          </div>
        </div>
        <div class="filter-row">
          <span class="filter-label">场景</span>
          <div class="filter-chips">
            <button type="button" :class="['filter-chip', { 'filter-chip--active': scene === 'all' }]" @click="scene = 'all'">全部</button>
            <button v-for="item in visibleScenes" :key="item" type="button" :class="['filter-chip', { 'filter-chip--active': scene === item }]" @click="scene = item">{{ item }}</button>
            <button v-if="scenes.length > sceneLimit" type="button" class="filter-chip filter-chip--more" @click="showAllScenes = !showAllScenes">{{ showAllScenes ? '收起' : `更多 ${scenes.length - sceneLimit}` }}</button>
          </div>
        </div>
      </div>
      <button type="button" :class="['favorites-filter', { 'favorites-filter--active': favoritesOnly }]" @click="favoritesOnly = !favoritesOnly">{{ favoritesOnly ? '已仅看收藏' : '仅看收藏' }}</button>
    </section>
...
const visibleCategories = computed(() => showAllCategories.value ? categories : categories.slice(0, categoryLimit));
const visibleStyles = computed(() => showAllStyles.value ? styles : styles.slice(0, styleLimit));
const visibleScenes = computed(() => showAllScenes.value ? scenes : scenes.slice(0, sceneLimit));
...
function resetFilters() { query.value = ''; category.value = 'all'; style.value = 'all'; scene.value = 'all'; favoritesOnly.value = false; showAllCategories.value = false; showAllStyles.value = false; showAllScenes.value = false; }
